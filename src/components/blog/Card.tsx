...
Card.Link = function CardLink({ children, ...props }: ComponentPropsWithoutRef<'a'>) {
  return (
    <>
      ...
      <a {...props}>
        <span className="absolute -inset-x-4 -inset-y-6 z-20 sm:-inset-x-6 rounded-sm sm:rounded-2xl" />
...
Card.Eyebrow = function CardEyebrow<T extends ElementType = 'p'>({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400/80',
        decorate && 'pl-3.5'
      )}
      {...props}
    >
